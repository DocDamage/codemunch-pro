...
class PcEngineRomCodec:
    ...
    def to_file_offset(self, location: AddressLocation) -> int | None:
        ...
        # Offset must be in 0x0000-0x1FFF range (8KB bank size)
        if offset > 0x1FFF:
            return None

        physical_addr = (bank * 0x2000) + offset

        # Physical address must be in valid range
        if physical_addr > 0x1FFFFF:
            return None
    ...
    def _parse_cpu(self, raw: str) -> AddressLocation | None:
        """Parse a CPU address (assumed bank 0)."""
        cleaned = raw.lower()
        if cleaned.startswith("0x"):
            cleaned = cleaned[2:]
        elif cleaned.startswith("\$"):
        ...
        # Must be in bank 0 range (0x0000-0x1FFF)
        if offset > 0x1FFF:
            return None
    ...
    def format(self, location: AddressLocation) -> str:
        """Format an AddressLocation back to canonical PC Engine ROM form."""
        if location.address_space != self.name:
            raise ValueError(f"unsupported address space: {location.address_space}")
    ...
    def to_file_offset(self, location: AddressLocation) -> int | None:
        """Convert PC Engine ROM location to file offset."""
        if location.address_space != self.name:
            return None
        ...
        # Fallback: calculate from bank and offset
        bank = location.attributes.get("bank")
        offset = location.attributes.get("offset")
        if isinstance(bank, int) and isinstance(offset, int):
            return (bank * 0x2000) + offset
...
class C64PrgCodec:
    ...
    The C64 uses a 6502 CPU with 64KB address space (0x0000-0xFFFF).
    PRG files have a 2-byte header containing the load address (low byte, high byte),
    followed by the data to be loaded.

    Address mapping:
        - C64 address space: 0x0000-0xFFFF (64KB)
        - PRG header: 2 bytes (load address, little-endian)
        - File offset = (c64_address - load_address) + 2

    Common load addresses:
        - 0x0801: BASIC program start (with BASIC stub)
        - 0x2000: Common ML (machine language) load address
        - 0x4000: Common ML load address
        - 0x8000: Common ML load address
        - 0xC000: Common ML load address

    Address formats:
        - "" (4 hex digits with dollar sign, canonical form)
        - "0xXXXX" (4 hex digits with 0x prefix)

    The codec stores the load address in the location attributes, allowing
    file offsets to be calculated correctly for any PRG file.

    Examples:
        "" with load_address=0x0801 -> file offset 2 (first data byte)
        "" with load_address=0x0801 -> file offset 3
        "" with load_address=0x2000 -> file offset 2

    C64 Memory Map Reference:
        0x0000-0x00FF: Zero Page
        0x0100-0x01FF: Stack
        0x0200-0x03FF: BASIC input buffer
        0x0400-0x07FF: Screen memory
        0x0800-0x9FFF: BASIC RAM
        0xA000-0xBFFF: BASIC ROM or RAM
        0xC000-0xCFFF: RAM
        0xD000-0xDFFF: I/O or Character ROM
        0xE000-0xFFFF: KERNAL ROM or RAM
    """
    ...
    def __post_init__(self) -> None:
        # Pattern for C64 addresses: \, \, 0xC000, etc.
        # Matches \$ or 0x prefix followed by 1-4 hex digits
        self._address_pattern = re.compile(
            r"(?<![A-Za-z0-9_])(?:\$|0x)([0-9A-Fa-f]{1,4})(?![A-Za-z0-9_])",
            re.IGNORECASE
        )
    ...
    def extract_refs(self, text: str) -> list[str]:
        """Extract C64 PRG address references from text.

        Matches patterns like:
            - "", "", "" (with dollar sign)
            - "0x0400", "0x9FFF" (with 0x prefix)

        Returns:
            List of matched address strings in canonical form (\)
        """
        results = []
        ...
        for match in self._address_pattern.finditer(text):
            ...
            try:
                addr = int(addr_str, 16)
                # Validate address is in valid C64 address space (0x0000-0xFFFF)
                if 0x0000 <= addr <= 0xFFFF:
                    results.append(f"${addr:04X}")
            except ValueError:
                continue
    ...
    def parse(self, ref: str, *, load_address: int | None = None) -> AddressLocation | None:
        """Parse a C64 PRG reference into an AddressLocation.

        Args:
            ref: A reference string like "$0801", "0x2000", or "$C000"
            load_address: Optional load address from PRG header. If not provided,
                         uses the default_load_address (0x0801).

        Returns:
            AddressLocation with address type and mapping info in attributes,
            or None if invalid
        """
        raw = ref.strip().replace("_", "")
        ...
        # Validate address range (16-bit, 0x0000-0xFFFF)
        if not 0x0000 <= address <= 0xFFFF:
            return None

        # Use provided load address or default
        load_addr = load_address if load_address is not None else self.default_load_address
    ...
    def _get_address_type(self, address: int) -> str:
        """Determine the address type based on the C64 memory map."""
        if address <= 0x00FF:
            return "zero_page"
        elif address <= 0x01FF:
            return "stack"
    ...
    def _calculate_file_offset(self, address: int, load_address: int) -> int | None:
        """Calculate file offset from C64 address and load address.

        File offset = (c64_address - load_address) + 2

        The +2 accounts for the 2-byte PRG header containing the load address.
        Returns None if the address is before the load address.
        """
        if address < load_address:
            return None
        return (address - load_address) + 2
    ...
    def format(self, location: AddressLocation) -> str:
        """Format an AddressLocation back to canonical C64 PRG form (\)."""
        if location.address_space != self.name:
            raise ValueError(f"unsupported address space: {location.address_space}")
    ...
    def to_file_offset(self, location: AddressLocation) -> int | None:
        """Convert C64 PRG location to file offset.

        Uses the load_address stored in the location attributes.
        """
        if location.address_space != self.name:
            return None
...
@dataclass(slots=True)
class XboxXbeCodec:
    """Original Xbox XBE address codec."""
    ...
    def _parse_linear(self, raw: str) -> AddressLocation | None:
        """Parse a linear address in cartridge range."""
        cleaned = raw.lower()
        if cleaned.startswith("0x"):
            cleaned = cleaned[2:]
        elif cleaned.startswith("$"):
        ...
        # Address must be in cartridge range 0x1000-0x1FFF
        if not 0x1000 <= address <= 0x1FFF:
            return None
    ...
    def _parse_banked(self, raw: str) -> AddressLocation | None:
        """Parse a banked address in "B:ADDR" format."""
        parts = raw.split(":", 1)
        if len(parts) != 2:
            return None
        ...
        # Address must be in cartridge range
        if not 0x1000 <= address <= 0x1FFF:
            return None
    ...
    def _get_cart_type(self) -> str:
        """Determine cartridge type based on size."""
        if self.cart_size <= 0x800:
            return "2KB"
        elif self.cart_size <= 0x1000:
            return "4KB"
    ...
    def _calculate_file_offset(self, address: int) -> int | None:
        """Calculate file offset from Atari 2600 ROM address.

        For standard 4KB carts: offset = (address - 0x1000)
        For 2KB carts: handle mirror mapping
        """
        # Normalize address to 0x1000-0x1FFF range
        if address < 0x1000 or address > 0x1FFF:
            return None

        if self.cart_size == 0x800:  # 2KB cart
            # 2KB carts mirror at 0x1800-0x1FFF
            # Address & 0x7FF gives offset within 2KB bank
            return address & 0x7FF
        else:  # 4KB cart (standard) or larger
            return address - 0x1000

    def _calculate_banked_file_offset(self, bank: int, address: int) -> int | None:
        """Calculate file offset for banked cartridges.

        Parker Bros (8K): 4 banks of 2KB each (0x800)
        Tigervision (32K): 16 banks of 2KB each (0x800)
        F8 (Atari 8K): 2 banks of 4KB each (0x1000)
        F6 (Atari 16K): 4 banks of 4KB each (0x1000)
        """
        bank_size = self._get_bank_size()
        if bank_size == 0:
            return None
    ...
    def _get_bank_size(self) -> int:
        """Get bank size based on bankswitch type and cart size."""
        if self.bankswitch_type in ("parker_bros", "tigervision"):
            return 0x800  # 2KB banks
        elif self.bankswitch_type == "f8":
            return 0x1000  # 4KB banks
        elif self.bankswitch_type == "f6":
            return 0x1000  # 4KB banks
        elif self.cart_size > 0x1000:
            # Default to 2KB banks for larger carts
            return 0x800
        else:
            return self.cart_size

    def format(self, location: AddressLocation) -> str:
        """Format an AddressLocation back to canonical Atari 2600 ROM form."""
        if location.address_space != self.name:
            raise ValueError(f"unsupported address space: {location.address_space}")
    ...
    def to_file_offset(self, location: AddressLocation) -> int | None:
        """Convert Atari 2600 ROM location to file offset."""
        if location.address_space != self.name:
            return None
        ...
        # For standard 4KB carts, just subtract 0x1000
        if isinstance(bank, int):
            return self._calculate_banked_file_offset(bank, address)

        # Standard cart: offset = address - 0x1000
        if address < 0x1000 or address > 0x1FFF:
            return None
        return address - 0x1000
...
class Ps2ElfCodec:
    ...
    def _get_memory_region(self, addr: int) -> tuple[str, int, str]:
        if 0x00000000 <= addr <= 0x01FFFFFF:
            return ("ee_ram", addr, addr)
        if 0x20000000 <= addr <= 0x21FFFFFF:
            normalized = addr - 0x20000000
            return ("ee_ram_mirror1", normalized, addr - 0x20000000)
        if 0x30000000 <= addr <= 0x31FFFFFF:
            normalized = addr - 0x30000000
            return ("ee_ram_mirror2", normalized, addr - 0x30000000)
        if 0xBC000000 <= addr <= 0xBC1FFFFF:
            return ("iop_ram", addr, addr)
        if 0x12000000 <= addr <= 0x1200FFFF:
            return ("gs_registers", addr, addr)
        return ("unknown", addr, addr)

    def _calculate_file_offset(self, addr: int, region: str, load_base: int | None = None, segment_offset: int | None = None) -> int | None:
        if region not in ("ee_ram", "ee_ram_mirror1", "ee_ram_mirror2"):
            return None
        if load_base is None:
            load_base = self.default_ee_load_base
    ...
    def to_file_offset(self, location: AddressLocation, load_base: int | None = None, segment_file_offset: int | None = None) -> int | None:
        if location.address_space != self.name:
            return None
        address = location.attributes.get("address")
        region = location.attributes.get("memory_region")
